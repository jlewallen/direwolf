...
void kissnet_raw_send(int Client, unsigned char * ACK, int acklen)
{
	int err;
...
#if __WIN32__	
	        err = send (client_sock[Client], (char*)ACK, acklen , 0);
			if (err == SOCKET_ERROR)
			{
				text_color_set(DW_COLOR_ERROR);
...
#else
			err = SOCK_SEND (client_sock[Client], (char*)ACK, acklen);
			if (err <= 0)
			{
				perror("raw");
				text_color_set(DW_COLOR_ERROR);
